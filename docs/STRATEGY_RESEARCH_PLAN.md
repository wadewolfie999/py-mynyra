...
1. Verify FIBO's XAUUSD symbol, historical M1 availability, quote timestamps,
   contract units and trading costs. A short connection sample checks the data
   path; it is not enough to judge a strategy.
...
6. Freeze selected rules and assess them on the reserved final period. Move to
   forward observation and bounded demo execution if evidence justifies it.

Research can progress alongside data preparation. Personal spending decisions and
provider selection do not need to be resolved to research or simulate candidates.
...
Next deliverables: verified XAUUSD data/cost information, a source-backed shortlist,
and exact test rules. The owner need not invent or supply a strategy to proceed.
